...
class PromoGiver:
    ...
    async def give_promo(self, user_id: int, promo_id: int, session: AsyncSession):
        stmt = sa.select(Promotion).where(Promotion.id == promo_id, Promotion.source == self.giver)
        ...
        for promo in existing_promos:
            if self._is_conflicting(promo.promotion.type, promotion.type):
